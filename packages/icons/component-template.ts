...
export const getComponentName = (name: string): string => {
  const suffix = name === 'Props' ? '_Props' : name;
  return `Icon${suffix}`;
};
...
export const ${iconDataVariableName} = {
  ${images
    .map(
      image => `"${image.variant}": {
    imageSrc: ${image.id},
...
export interface ${componentPropsName} extends IconProps {
  /** Icon variant to use, also provides default styling with the correct height and width **/
  variant?: ${images.map(image => `"${image.variant}"`).join(' | ')};
}

export const ${componentName}: React.FC<${componentPropsName}> = ({variant = "${
    images[0].variant
  }", ...rest}) => {
